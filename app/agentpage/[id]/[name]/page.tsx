  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="flex flex-1">
        {/* Sidebar */}
         <Sidebar agents={agents} agentId={id as string} agentName={agentName} />
                ...
                <tbody>
                  {topics.map((topic) => (
                    <tr key={topic.id} className="border-t hover:bg-gray-50">
                      ...
                      <td
  onClick={() => router.push(`/topic/details/${topic.id}?name=${encodeURIComponent(topic.name.trim())}`)}
  className="px-4 py-2 text-purple-700 font-medium cursor-pointer hover:underline"
>
  {topic.name}
</td>
